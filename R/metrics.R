...
estMets <- function(mse, specdat, set){
    ...
    mets <- c("BBmsyFL","avCatch","avCatchLast5y","BBmsyLowest",
              "PBBlim","PBBlimLast5y","CatchCV", "avRelCatch",
    ...
    for(i in 1:nmse){
        ...
        ## "avRelCatch"
        if("ref" %in% set$mss){
            tmpRef <- lapply(resMSE[[which(set$mss == "ref")]], function(x) x$CW[simYears])
        ...
        ## "avCRelatchLast5y"
        if("ref" %in% set$mss){
            tmpRef <- lapply(resMSE[[which(set$mss == "ref")]], function(x) x$CW[last5Years])
        ...
        res[10,] <- c(NA, (1 - (sum(tmp) / (nysim * nrep))) * 100, NA)
        ## "avCatchFirst5y"
        tmp <- unlist(lapply(msei, function(x) x$CW[first5Years]))
        res[11,] <- quantile(tmp, probs = c(0.025, 0.5, 0.975), na.rm=TRUE)
        ## "avRelCatchFirst5y"
        if("ref" %in% set$mss){
            tmpRef <- lapply(resMSE[[which(set$mss == "ref")]], function(x) x$CW[first5Years])
