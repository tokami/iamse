...
estMets <- function(mse, dat, mets = "all"){
    ...
    res2 <- vector("list", nspec)
    for(spec in 1:nspec){
        res2[[spec]] <- vector("list", nscen)
        ## TODO: make part of mse
        refs <- dat[[spec]]$ref
        ##
        for(scen in 1:nscen){
            res2[[spec]][[scen]] <- vector("list", nhcr)
            for(hcr in 1:nhcr){
                nrep <- length(mse[[spec]][[scen]][[hcr]])
                msei <- mse[[spec]][[scen]][[hcr]]
                res <- NULL
                ## "BBmsyFL"
                if(any(mets == "BBmsyFL")){
                ...
                res2[[spec]][[scen]][[hcr]] <- round(res, 3)
            }
            names(res2[[spec]][[scen]]) <- hcrs
        }
    }
